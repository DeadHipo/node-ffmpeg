...
module.exports = function (filePath, settings, infoConfiguration, infoFile) {
	...
	this.setDisableAudio = function () {
		if (options.audio == undefined) options.audio = new Object()
		// Set the new option
	...
	this.setVideoFormat = function (format) {
		...
		if (this.info_configuration.encode.indexOf(format) != -1) {
			...
			return this
		} else throw errors.renderError('format_not_supported', format)
	}
	...
	this.setVideoCodec = function (codec) {
		...
		if (this.info_configuration.encode.indexOf(codec) != -1) {
			...
			return this
		} else throw errors.renderError('codec_not_supported', codec)
	}
	...
	this.setAudioCodec = function (codec) {
		...
		if (this.info_configuration.encode.indexOf(codec) != -1) {
			...
			if (
				codec == 'mp3' &&
				this.info_configuration.modules.indexOf('libmp3lame') != -1
			)
				codec = 'libmp3lame'
	...
	this.setAudioChannels = function (channel) {
		...
		if (
			presets.audio_channel.stereo == channel ||
			presets.audio_channel.mono == channel
		) {
			if (options.audio == undefined) options.audio = new Object()
			// Set the new option
			options.audio.channel = channel
			return this
		} else throw errors.renderError('audio_channel_is_invalid', channel)
	}
	...
	this.setWatermark = function (watermarkPath, settings) {
		...
		if (settings != null) utils.mergeObject(baseSettings, settings)

		// Check if position is valid
		if (
			baseSettings.position == null ||
			utils.in_array(baseSettings.position, [
				'NE',
				'NC',
				'NW',
				'SE',
		...
		var getSing = function (val, inverse) {
			return (
				(val > 0 ? (inverse ? '-' : '+') : inverse ? '+' : '-').toString() +
			...
			case 'NE':
				overlay =
					...
					getHorizontalMargins(
						...
						baseSettings.margin_west
					) +
					':0' +
					getVerticalMargins(baseSettings.margin_nord, baseSettings.margin_sud)
				break
			case 'NC':
				overlay =
				...
				if (options.video.hasOwnProperty('size')) {
					...
					if (newDimension.aspect != null) {
						this.addFilterComplex(
							'scale=iw*sar:ih, pad=max(iw\\,ih*(' +
								newDimension.aspect.x +
								'/' +
								newDimension.aspect.y +
								')):ow/(' +
								newDimension.aspect.x +
								'/' +
								newDimension.aspect.y +
								'):(ow-iw)/2:(oh-ih)/2' +
								(options.video.paddingColor != null
									? ':' + options.video.paddingColor
									: '')
						)
	...
	var _calculateNewDimension = function () {
		...
		var keepPixelAspectRatio =
			typeof options.video.keepPixelAspectRatio != 'boolean'
				? false
				: options.video.keepPixelAspectRatio
		// Check if keepAspectRatio is undefined
		var keepAspectRatio =
			typeof options.video.keepAspectRatio != 'boolean'
				? false
				: options.video.keepAspectRatio
		...
		if (fixedWidth) {
			...
			if (!utils.isEmptyObj(this.metadata.video.aspect)) {
				height = Math.round(
					(width / this.metadata.video.aspect.x) * this.metadata.video.aspect.y
				)
			} else {
				// Calculte the new height
				height = Math.round(
					referrerResolution.h /
						(referrerResolution.w / parseInt(fixedWidth[1], 10))
				)
			}
		} else if (fixedHeight) {
			// Set the width dimension
			...
			if (!utils.isEmptyObj(this.metadata.video.aspect)) {
				width = Math.round(
					(height / this.metadata.video.aspect.y) * this.metadata.video.aspect.x
				)
			} else {
				// Calculte the new width
				width = Math.round(
					referrerResolution.w /
						(referrerResolution.h / parseInt(fixedHeight[1], 10))
				)
			}
		} else if (percentage) {
		...
		} else throw errors.renderError('size_format', options.video.size)
	...
	var execCommand = function (callback, folder) {
		...
		var ffmpegBin = process.env.FFMPEG_PATH || 'ffmpeg'
		// Create a copy of the commands list
		var finalCommands = [`${ffmpegBin} -i`]
			.concat(inputs.join(' -i '))
			.concat(commands.join(' '))
			.concat(
				filtersComlpex.length > 0
					? ['-filter_complex "'].concat(filtersComlpex.join(', ')).join('') +
							'"'
					: []
			)
			.concat([output])
		...
		utils.exec(finalCommands, settings, function (error, stdout, stderr) {
			...
			if (!error) {
				...
				if (onlyDestinationFile) {
					...
					for (var i in result) result[i] = [folder, result[i]].join('/')
				}
			}
	...
	this.fnExtractSoundToMP3 = function (destionationFileName, callback) {
		...
		if (fs.existsSync(destionationFileName)) fs.unlinkSync(destionationFileName)

		// Building the final path
		var destinationDirName = path.dirname(destionationFileName),
			destinationFileNameWE =
				path.basename(
					destionationFileName,
					path.extname(destionationFileName)
				) + '.mp3',
	 ...
	 */
	this.fnExtractFrameToJPG =
		function (/* destinationFolder, settings, callback */) {
			...
			// Check if the settings are specified
			if (newSettings !== null) utils.mergeObject(settings, newSettings)
			...
			if (settings.duration_time != null) {
				if (/([0-9]+):([0-9]{2}):([0-9]{2})/.exec(settings.duration_time))
					settings.duration_time = utils.durationToSeconds(
						settings.duration_time
					)
				else if (!isNaN(settings.duration_time))
					settings.duration_time = parseInt(settings.duration_time, 10)
				else settings.duration_time = null
			}
			...
			if (settings.size == null)
				settings.size =
					this.metadata.video.resolution.w +
					'x' +
					this.metadata.video.resolution.h
			...
			if (
				...
				(isNaN(settings.every_n_percentage) ||
			...
			if (settings.file_name == null) {
				settings.file_name = path.basename(
					this.file_path,
				...
				if (replacements) {
					for (var i in replacements) {
						switch (replacements[i]) {
							case '%t':
								settings.file_name = settings.file_name.replace(
									'%t',
									new Date().getTime()
								)
								break
							...
							case '%x':
								settings.file_name = settings.file_name.replace(
									'%x',
									settings.size.split(':')[0]
								)
								break
							case '%y':
								settings.file_name = settings.file_name.replace(
									'%y',
									settings.size.split(':')[1]
								)
								break
							default:
								settings.file_name = settings.file_name.replace(
			...
			settings.file_name =
				path.basename(settings.file_name, path.extname(settings.file_name)) +
				'_%d.png'
			...
			// Adding commands to the list
			if (settings.start_time) this.addCommand('-ss', settings.start_time)
			if (settings.duration_time) this.addCommand('-t', settings.duration_time)
			if (settings.frame_rate) this.addCommand('-r', settings.frame_rate)

			// Setting the size and padding settings
			this.setVideoSize(
				settings.size,
				settings.keep_pixel_aspect_ratio,
			...
			if (settings.number) this.addCommand('-vframes', settings.number)
			if (settings.every_n_frames) {
				this.addCommand('-vsync', 0)
				this.addFilterComplex(
					'select=not(mod(n\\,' + settings.every_n_frames + '))'
				)
			}
			if (settings.every_n_seconds) {
				this.addCommand('-vsync', 0)
				this.addFilterComplex(
					'select=not(mod(t\\,' + settings.every_n_seconds + '))'
				)
			}
			if (settings.every_n_percentage) {
				...
				this.addFilterComplex(
					'select=not(mod(t\\,' +
						parseInt(
							(this.metadata.duration.seconds / 100) *
	 ...
	 */
	this.fnAddWatermark = function (
		watermarkPath /* newFilepath , settings, callback */
	) {
		...
		if (newFilepath == null)
			newFilepath =
				...
				'/' +
				path.basename(this.file_path, path.extname(this.file_path)) +
				'_watermark_' +
