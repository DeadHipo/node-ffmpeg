...
var ffmpeg = function (/* inputFilepath, settings, callback */) {
			...
			function (error, stdout, stderr) {
				...
				if (configuration) {
					...
					for (var indexModule in modules) {
						...
						if (modules[indexModule].length > 0) {
							format.modules.push(
								/--enable-([a-zA-Z0-9\-]+)/.exec(modules[indexModule])[1]
							)
						}
					}
				}
	...
	var _videoInfo = function (fileInput, settings) {
		...
		// Make the call to retrieve information about the ffmpeg
		utils.exec(
			[ffmpeg.bin, '-i', utils.addQuotes(fileInput), '2>&1'],
			settings,
			function (error, stdout, stderr) {
				...
				var filename = /from \'(.*)\'/.exec(stdout) || [],
					...
					duration =
						/Duration: (([0-9]+):([0-9]{2}):([0-9]{2}).([0-9]+))/.exec(
							stdout
						) || [],
				...
				var ret = {
					...
					video: {
						...
						bitrate:
							video_bitrate.length > 1 ? parseInt(video_bitrate[1], 10) : 0,
					...
					audio: {
						...
						sample_rate:
							sample_rate.length > 1 ? parseInt(sample_rate[1], 10) : 0,
						stream: audio_stream.length > 1 ? parseFloat(audio_stream[1]) : 0.0,
						channels: {
							raw: channels[1] || '',
							value:
								channels.length > 0
									? { stereo: 2, mono: 1 }[channels[1]] || 0
									: '',
						},
					},
				}
				...
				if (ret.video.pixel !== 1 || ret.video.pixel !== 0) {
					if (ret.video.pixel > 1) {
						ret.video.resolutionSquare.w = parseInt(
							ret.video.resolution.w * ret.video.pixel,
							10
						)
						ret.video.resolutionSquare.h = ret.video.resolution.h
					} else {
						ret.video.resolutionSquare.w = ret.video.resolution.w
						ret.video.resolutionSquare.h = parseInt(
							ret.video.resolution.h / ret.video.pixel,
							10
						)
					}
				}
	...
	var __constructor = function (args) {
		...
		when(
			_getInformation(inputFilepath, settings),
			function (data) {
